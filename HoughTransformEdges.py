...
images_batch_edges = torch.empty((113, 3, 100, 100))
...
def hough_transform():
    for imagePath, i in zip(os.listdir(inPath), range(113)):
        ...
        img1 = torch.squeeze(images_batch[i]).moveaxis(0, 2).numpy()
        img2 = np.uint8(img1)

        img = cv2.cvtColor(img2, cv2.COLOR_RGB2GRAY)
        # print(img.shape, img)

        prova1 = cv2.Canny(img, 50, 150)
        prova2 = cv2.Canny(img, 100, 250)
        prova3 = cv2.Canny(img, 200, 350)

        # print(type(prova1), prova1.shape)

        images_batch_edges[i, 0, :, :] = torch.from_numpy(prova1).unsqueeze_(0).unsqueeze_(0).type(torch.float32)
        images_batch_edges[i, 1, :, :] = torch.from_numpy(prova2).unsqueeze_(0).unsqueeze_(0).type(torch.float32)
        images_batch_edges[i, 2, :, :] = torch.from_numpy(prova3).unsqueeze_(0).unsqueeze_(0).type(torch.float32)
    ...
    return images_batch_edges                                             # prova1, prova2, prova3, img


# code to print some results as examples
def plot_examples():
    for image, i in zip(images_batch_edges, range(113)):
        ...
        if i in range(5):
            ...
            plt.imshow(images_batch_edges)
            plt.show()


# main for some testing
'''
def main():
    r1, r2, r3, r4 = hough_transform()
